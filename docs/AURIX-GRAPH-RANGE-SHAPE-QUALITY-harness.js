...
const sgap=build('24h-gap'); const rgap=render('24h-gap', sgap);
ok('10 gap detected + line split (not solid across hole)', (rgap.gaps||[]).length>=1 && rgap.diagnostics.renderedSubpaths>=2, 'gaps='+(rgap.gaps||[]).length+' subpaths='+rgap.diagnostics.renderedSubpaths);
