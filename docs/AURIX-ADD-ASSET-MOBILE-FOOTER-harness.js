...
ok('3.3 footer background is OPAQUE Aurix surface (not a translucent gradient)', (function () {
  const m = /\.add-v2-footer\s*\{[^}]*background:\s*rgba\(\d+,\d+,\d+,(0?\.\d+|1)\)/.exec(fb);
  return !!m && parseFloat(m[1]) >= 0.9 && !/\.add-v2-footer\s*\{[^}]*background:\s*linear-gradient/.test(fb);
})());
