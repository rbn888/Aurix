...
ck('G1.4 the institutional WEB engine is used by the lite renderer as a PURE path source (no DOM)',
   // window widened: BLOCK-B added the LKG republish branch inside this function (no DOM, no engine change).
   /renderAurixMobileLiteChart[\s\S]{0,10200}renderValidatedPortfolioChartWithInstitutionalRenderer\(/.test(app) ||
   /renderAurixMobileLiteChart[\s\S]{0,12000}renderAurixInstitutionalChart\(/.test(app));
