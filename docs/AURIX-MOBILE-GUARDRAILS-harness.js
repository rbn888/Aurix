...
ck('G1.4 the institutional WEB engine is used by the lite renderer as a PURE path source (no DOM)',
   /renderAurixMobileLiteChart[\s\S]{0,9200}renderValidatedPortfolioChartWithInstitutionalRenderer\(/.test(app) ||
   /renderAurixMobileLiteChart[\s\S]{0,11000}renderAurixInstitutionalChart\(/.test(app));
