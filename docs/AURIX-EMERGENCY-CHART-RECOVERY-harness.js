...
// BLOCK-B — the badge reads the published result for the active range first (so the % always belongs
// to the series on screen) and builds a production chart object only when none is published yet.
ok('W3 return badge painter routes through the production chart object', /if \(_aurixEmergencyChartOn\(\)\) \{[\s\S]{0,900}?_aurixEmergencyPaintBadgeNode\(el, _pub \|\| buildProductionPortfolioChart\(_r\), surface\)/.test(app));
