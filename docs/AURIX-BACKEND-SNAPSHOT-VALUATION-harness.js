...
ok('app.js hydration ACTIVATED, auth-gated, retryable (failure→null so it never falsely completes)', /const _AURIX_BACKEND_SNAPSHOTS_AUTOLOAD = true;/.test(app) && /if \(error \|\| !Array\.isArray\(_rows\)\) return null;/.test(app));   // SPEC P0 HISTORICAL CONTINUITY: la lectura es ahora paginada; el contrato (fallo⇒null) es el mismo, por página
