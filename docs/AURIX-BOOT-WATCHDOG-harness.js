...
{ const idx = fs.readFileSync(path.join(root, 'index.html'), 'utf8');
  ...
  // hardcoded number), so a routine build bump never makes this assertion stale.
  ck('index.html: app.js tag version matches APPJS_V + load-error diagnostic', (() => {
    const mv = idx.match(/var APPJS_V = '(\d+)';/);
    return !!mv && new RegExp('app\\.js\\?v=' + mv[1]).test(idx) && idx.indexOf('app_js_load_error') >= 0;
  })());
