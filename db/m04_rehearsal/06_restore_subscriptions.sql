-- ============================================================================
-- M.04 ENSAYO · PASO 6 — RESTAURAR public.subscriptions AL ESTADO PRE
-- ----------------------------------------------------------------------------
-- *** NO EJECUTADO. Artefacto preparatorio. ***
-- psql -v ON_ERROR_STOP=1 -f db/m04_rehearsal/06_restore_subscriptions.sql
--
...
-- y el VERIFY POST (paso 7) es el que decide si el resultado vale. Si este
...
begin
  ...
  if not found or v_g.verdict <> 'PASS' or v_g.run_id <> v_run then
    raise exception 'RESTORE = BLOCKED: VERIFY_PRE no es PASS para este run (paso 3).';
  end if;
...
-- es el paso 7. Esta afirmación de aquí sólo evita hacer commit de una
